...
def update_cellar_image_status(db: Session, image_id: str, status: str):
    row = db.execute(
        text("""
...
def update_slot_geometry(db: Session,slot_id: str,polygon_json=None,bbox_json=None,
                         label: str | None = None,status: str | None = None,):
...
def delete_slots_by_image(db: Session, image_id: str):
    result = db.execute(
        text("""
